...
from .security_mw import ReplayGuard, RateLimiter
...
replay = ReplayGuard(ttl_s=int(os.getenv('BL17_REPLAY_TTL_SECONDS','600')))
rl = RateLimiter(per_minute=int(os.getenv('BL17_RATE_LIMIT_PER_MINUTE','120')))
...
def create_text(inp: TextIntentIn, db: Session = Depends(db_dep)) -> IntentDraft:
    ...
    fp = replay.fingerprint(inp.actor_id, 'TEXT', {'text': inp.text})
    ok, rreason = replay.check_and_mark(fp)
    if not ok:
        raise HTTPException(status_code=409, detail=rreason)
...
def create_voice(inp: VoiceIntentIn, db: Session = Depends(db_dep)) -> IntentDraft:
    if not rl.allow(inp.actor_id):
        raise HTTPException(status_code=429, detail='rate_limited')
    action_type, target, params = parse_intent(inp.transcript_text)
    fp = replay.fingerprint(inp.actor_id, 'VOICE', {'transcript_text': inp.transcript_text})
    ok, rreason = replay.check_and_mark(fp)
    if not ok:
        raise HTTPException(status_code=409, detail=rreason)
    iid = f"intent_{uuid.uuid4().hex}"
