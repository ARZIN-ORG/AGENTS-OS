...
class SuggestionEnvelope(BaseModel):
    # Non-executable recommendation artifact
    ...
    # informational only; must NOT auto-trigger execution
    confidence: float = Field(..., ge=0.0, le=1.0)

    expected_impact: Dict[str, Any] = Field(default_factory=dict)
    risk_notes: List[str] = Field(default_factory=list)

    # Proposed changes/actions (still non-executable until human accepts)
    ...
    human_final_approval_id: Optional[str] = None  # links to BL-17/Permit audit trace
...
class SuggestionDecisionRequest(BaseModel):
    ...
    # If MODIFY, user can override proposed_action/title/summary
